...
/**
 * Api client-facing workout controller
 *
 * A client can check its already finished workouts, its today's workout, can finish exercises and complete workouts when all exercises are done.
 */
        ...
        try {
            ...
            foreach ($request->exercises as $index => $exerciseData) {
                WorkoutExercise::create([
                    ...
                    'order'         => $index + 1,
                    'notes'         => $exerciseData['instructions'] ? implode(', ', $exerciseData['instructions']) : '',
                    'completed'     => false
                ]);
            }
        ...
        try {
            ...
            foreach ($exercises as $index => $exercise) {
                WorkoutExercise::create([
                    ...
                    'rest_time' => 60,
                    'order' => $index + 1,
                    'notes' => $exercise['instructions'] ? implode(', ', $exercise['instructions']) : '',
                    'completed' => false
                ]);
            }
